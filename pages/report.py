...
@callback(
    Output("report-bridge-selector", "value"),
    Input("current-bridge-store", "data"),
    State("report-bridge-selector", "value"),
)
def sync_report_bridge_selector(store_data, current_value):
...
def on_report_bridge_change(bridge_id, current_store, bridge_trigger, home_trigger):
    if not bridge_id:
